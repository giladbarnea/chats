...
def test_short_modifier_claude_session(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    session_path = temp_home / ".claude" / "projects" / "p1" / "1e446a9f-08fd-43ac-be72-8ce337d01dcd.jsonl"
    ...
    _write_jsonl(session_path, [
        {
            ...
            "message": {
                "role": "assistant",
                "content": [{"type": "tool_use", "id": "t1", "name": "Bash", "input": {"command": long_text}}]
            }
        }
    ])
...
def test_short_modifier_codex_session(tmp_path: Path, monkeypatch, capsys) -> None:
    temp_home = tmp_path / "home"
    monkeypatch.setattr(Path, "home", lambda: temp_home)
    session_path = temp_home / ".codex" / "sessions" / "2026" / "04" / "10" / "rollout-session001.jsonl"
...
def test_thinking_short_modifier_claude_session(tmp_path: Path, monkeypatch, capsys) -> None:
    temp_home = tmp_path / "home"
    monkeypatch.setattr(Path, "home", lambda: temp_home)
    session_path = temp_home / ".claude" / "projects" / "p1" / "thinking-short-claude.jsonl"
...
def test_thinking_short_modifier_codex_session(tmp_path: Path, monkeypatch, capsys) -> None:
    temp_home = tmp_path / "home"
    monkeypatch.setattr(Path, "home", lambda: temp_home)
    session_path = temp_home / ".codex" / "sessions" / "2026" / "04" / "10" / "thinking-short-codex.jsonl"
...
def test_thinking_short_modifier_pi_session(tmp_path: Path, monkeypatch, capsys) -> None:
    temp_home = tmp_path / "home"
    monkeypatch.setattr(Path, "home", lambda: temp_home)
    session_path = (
        temp_home
