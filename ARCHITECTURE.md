...
│                               Bare uppercase AND/OR/NOT tokens → boolean tree
...
│      │                        ├── _search_path_candidate_matches(path, ...)
│      │                        │   └── Native safe skip for plain-literal misses
...
7. **Candidate Pass Is an Optimization, Not New Semantics**: eligible plain-literal queries first go through `_search_path_candidate_matches()`, but every surviving file still gets the normal `SessionScan` rendered-content confirmation pass. The former decoded-content candidate gate is gone, so one semantic authority decides every result. Render-dependent and uncertain patterns bypass native rejection entirely.
...
23. **Search Has a Native Conservative Byte Candidate Gate**: `_search_path_candidate_matches()` rejects only safe ASCII literal misses before `read_text`. Rust scans raw files with bounded 1 MiB reads, preserves cross-chunk needles and evidence groups, and lowercases only ASCII haystack bytes for insensitive terms. Case-sensitive behavior remains unchanged. Under default unshortened visibility, case-insensitive scans continue across valid non-ASCII scalars that cannot create an ASCII match under Python 3.14. The 20 casefold or regex risks, invalid or incomplete UTF-8, raw JSON Unicode escapes, and default joined-Pi agent evidence defer to semantic confirmation. Non-default generated content, shortening, JSON-unstable queries, non-ASCII literals, regex terms, and render-dependent terms bypass native rejection. Every survivor goes directly to `_search_conversation_content()` and its sole `SessionScan` rendered-message authority.
...
30. **Resolution Facet File I/O Is Native**: after an exact identifier miss, Rust scans each candidate file forward, validates UTF-8, and sends only lines with raw title or summary markers to Python. The Python callback keeps `json.loads` and shared provider-title extraction authoritative. Rust returns the latest valid title and ordered summaries through the unchanged `extract_resolution_facets_from_jsonl()` interface. Python still owns title-before-summary matching and ambiguity handling.
