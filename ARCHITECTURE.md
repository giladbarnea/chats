...
28. **Antigravity Tool Results Pair by Record Type**: each pending call records its expected result type, and a result consumes the earliest compatible call instead of the next call blindly. An unmatched result gets a synthetic identity plus source-derived filter names. Ambiguous `CODE_ACTION` results carry candidate names rather than inventing one call identity.
