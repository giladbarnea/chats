...
The implementation stayed inside the existing tool-filter boundary: `ToolFilter` now stores an optional local short limit, `resolve_tool_visibility` chooses the most specific matching short declaration, and `Message._iter_visible_tools` consumes that resolved limit.
