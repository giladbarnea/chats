...
def adversarial_haystacks(pattern: str) -> list[str]:
    ...
    return [
        ...
        seed_text.upper(),
        seed_text + "\n" + seed_text,
        # A differently-cased repeat. Without it a backreference can never be
        # asked whether it folds case, because every other haystack repeats a
        # string in the same case it first appeared. That gap hid a real defect:
        # `(\w+)x\1` matched "PYxpy" case-sensitively, where CPython does not.
        seed_text + "x" + seed_text.upper(),
        "aZ9_ ß中\n",
        "",
        pattern[:12],
    ]
