...
Negative recent indices resolve across the unified Claude/PI/Codex session pool using the last in-band JSONL timestamp, so `-1` means “the newest supported session by transcript content”. If a session has no readable timestamp, it falls back to filesystem mtime for ordering. Claude agent sidechain files are excluded from this selector.
...
-t, --tools [SPEC]*  # Include tool use/result details. Filter with modifiers:
                     ...
                     #   Aliases: -t exec_command, -t shell_command
...
- Per-file List queries continuously refill up to six active file evaluations and commit results in scan order. Other per-file output modes retain ordered six-file windows. Eligible single-literal queries retain the existing batch gate. Closing colored paged List output stops new admission, then already-active reads drain
...
**Scope:** Claude and PI only. A Codex session is rejected with a clear error.
...
- Native search candidate rejection stays limited to plain ASCII literal queries with semantics that raw bytes can safely approximate. Regexes, non-default visible content, shortening, JSON-unstable queries, and uncertain Unicode still use the full rendered-content matcher
